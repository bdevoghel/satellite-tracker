...
try:
    from tracking import Led, Servo, Stepper
...
def track_satellite(
    sat, lat, lon, alt, duration, interval, testing=True, verbose=False
):
    try:
        # Set up motors
        ...
        if use_gpio:
            stepper = Stepper(
                direction_pin=20, step_pin=21, enable_pin=16, verbose=verbose
            )
        ...
        track_callback = partial(
            tracker, stepper=stepper, servo=servo, led=led, verbose=verbose
        )
        pointer.start_pointing(
            sat,
            lat,
            lon,
            alt,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Track a satellite's position with respect to an observer location."
    )
    parser.add_argument(
        "-s",
        "--sat",
        ...
        default=25544,
        type=int,
        help="NORAD id of satellite to track",
    )  # 25544 corresponds to ISS
    parser.add_argument(
        "-l",
        "--lat",
        action="store",
        default=0.0,
        type=float,
        help="Observer's latitide (decimal degrees format)",
    )
    parser.add_argument(
        "-L",
        "--lon",
        ...
        default=0.0,
        type=float,
        help="Observer's longitude (decimal degrees format)",
    )
    parser.add_argument(
        "-a",
        "--alt",
        ...
        default=0.0,
        type=float,
        help="Observer's altitude above sea level in meters",
    )
    parser.add_argument(
        "-d",
        "--duration",
        ...
        default=1,
        type=int,
        help="Duration of the tracking in seconds. -1 for indefinite",
    )
    parser.add_argument(
        "-i",
        "--interval",
        ...
        default=1.0,
        type=float,
        help="Interval between two track callbacks. Accurate to the second",
    )
    parser.add_argument(

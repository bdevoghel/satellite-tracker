...
class Stepper:
    def __init__(self, direction_pin, step_pin, enable_pin, verbose=False):
        ...
        self.SPR = 400  # steps per revolution (360/step_angle) TODO implement microstepping ?
    ...
    def point(self, angle, verbose=False):
        if verbose > 2:
            print(f"   - {__class__.__name__} pointing to {angle:.2f}°")
        
        # Determine orientation to take
        step_target = int((angle / 360.0 * self.SPR) % self.SPR)
        self.change_direction((((step_target + self.SPR - self.current_step) % self.SPR) < self.SPR/2) == self.CW) 

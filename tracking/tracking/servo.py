...
class Servo:
    def __init__(self, pwm_pin, mid_angle=90, verbose=False):
        ...
        assert (
            mid_angle in mid_angle_possibilities
        ), f"mid_angle must be one of {mid_angle_possibilities}"
        ...
        lgpio.tx_pwm(
            self.gpio_handle, pwm_pin, self.FREQ, 0
        )  # start PWM with pulse off
    ...
    def point(self, angle, verbose=False, jitter_delay_factor=1):
        ...
        logger.info(
            f"   - {__class__.__name__} pointing to {angle - 90 + self.mid_angle:.2f}°"
        )

        # Change duty cycle
        duty = min(
            self.DUTY_MAX,
            self.DUTY_MIN + angle / (180.0 / (self.DUTY_MAX - self.DUTY_MIN)),
        )
